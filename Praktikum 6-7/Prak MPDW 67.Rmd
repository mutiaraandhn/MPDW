...
```
Plot Boxcox menunjukkan nilai *rounded value* ($\lambda$) optimum sebesar **4** dan pada selang kepercayaan 95% nilai memiliki batas bawah **3.191919** dan batas atas **4**. Selang tersebut tidak memuat nilai satu sehingga dapat dikatakan bahwa data tidak stasioner dalam ragam.
...
eacf(train.diff)
```
Identifikasi model menggunakan plot EACF dilakukan dengan melihat ujung segitiga pada pola segitiga nol. Dalam hal ini model tentatif yang terbentuk adalah ARIMA(0,0,1) atau MA(1) dan ARIMA(1,0,2), atau ARMA (1,2).

### Pendugaan Parameter Model Tentatif

#### AR(4) atau ARIMA(4,0,0)
```{r}
model1.da=Arima(train.diff, order=c(2,1,12),method="ML")
...
```{r}
model2.da=Arima(train.ts, order=c(1,1,12),method="ML")
...
```{r}
model3.da=Arima(train.ts, order=c(1,1,0),method="ML")
summary(model3.da) #AIC=1603.61
